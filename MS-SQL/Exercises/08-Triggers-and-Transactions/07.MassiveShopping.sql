...
SET @itemsCost = (SELECT SUM(Price) FROM Items WHERE MinLevel IN (@minLevel, @maxLevel))

IF (@availableCash >= @itemsCost AND @userGameLevel >= @maxLevel)
BEGIN
    BEGIN TRANSACTION
		UPDATE UsersGames SET Cash -= @itemsCost
		WHERE Id = @userGameId
		IF (@@ROWCOUNT <> 1)
		BEGIN
			ROLLBACK
		...
		BEGIN
			...
			IF ((SELECT COUNT(*) FROM Items WHERE MinLevel IN (@minLevel, @maxLevel)) = @@ROWCOUNT)
			BEGIN
				ROLLBACK;
